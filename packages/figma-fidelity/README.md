...
| Tool                 | Description                                                               |
| -------------------- | ------------------------------------------------------------------------- |
| `fidelity_verify`    | Preferred 1–8 contract flow: fresh gold → stable capture → compare → evidence |
| `fidelity_done_gate` | Read-only completion check against exact hash-bound contracts             |
| `fidelity_status`    | Token, version, cwd, and effective project-root diagnostics                |

Low-level `fidelity_fetch_gold`, `fidelity_run`, `fidelity_capture`, `fidelity_compare`, and cache tools remain available through CLI or debug MCP. Set `FIGMA_FIDELITY_DEBUG_TOOLS=1` and restart client to expose them. Default surface omits cleanup/destructive maintenance tools.

`fidelity_verify` accepts project-relative `outDir` with explicit `projectRoot` (or `FIGMA_FIDELITY_PROJECT_ROOT`) and discriminated scope:
...
It stops each contract when fresh gold cannot be fetched, continues remaining contracts, returns structured partial results, and emits progress notifications. Use returned `allPassed` only as gate input; inspect every diff.
